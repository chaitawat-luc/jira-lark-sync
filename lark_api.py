"""Lark OpenAPI client (bot identity, no lark-cli needed)."""
import time
import requests
...
def fetch_all_records(token: str, base_token: str, table_id: str) -> list:
    """Return all records as list of {record_id, fields: {name: value}}."""
    records = []
    page_token = None
    while True:
...
def get_field_map(token: str, base_token: str, table_id: str) -> dict:
